...
// Client Hello callback SNI-based context switching
...
std::shared_ptr<boost::asio::ssl::context> getSslServerContext()
{
    ...
    if (c.tlsStrict)
    {
        ...
        BMCWEB_LOG_DEBUG("getSslServerContext: tlsStrict enabled ");
        if (ctxPair->mtlsCtx)
        {
    ...
    else if (!forward_unauthorized::hasWebuiRoute())
    {
        ...
        BMCWEB_LOG_DEBUG(
            "Setting verify peer only forward_unauthorized::hasWebuiRoute");
        if (ctxPair->httpsCtx)
        {

...
getSnmpTrapClientdata(const std::shared_ptr<bmcweb::AsyncResp> &asyncResp,
  ...
  crow::connections::systemBus->async_method_call(
      [asyncResp](
        ...
        for (const std::pair<std::string, std::variant<std::string, uint16_t>>
...
checkSnmpTrapClient(const std::shared_ptr<bmcweb::AsyncResp> &asyncResp,
  ...
  auto ret = std::from_chars(
      ...
      port.data(), port.data() + port.size(), snmpTrapPort);
  if (ret.ec != std::errc()) {
    messages::propertyValueTypeError(asyncResp->res, destUrl, "Destination");
    return;
  }
            ...
            if (interface.first == "xyz.openbmc_project.Network.Client") {
              ...
              if (address == host && portNum == snmpTrapPort) {
                messages::resourceAlreadyExists(
                    asyncResp->res, "EventDestination.v1_7_0.EventDestination",
                    "Destination", destUrl);
                return;
              }
            }
...
inline void requestRoutesEventDestinationCollection(App &app) {
                ...
                [asyncResp,
                 &memberArray](const boost::system::error_code ec,
                  ...
                  if (ec) {
                    ...
                    BMCWEB_LOG_ERROR
                        << "D-Bus response error on GetManagedObjects " << ec;
                    messages::internalError(asyncResp->res);
                    return;
                  }
  ...
  BMCWEB_ROUTE(app, "/redfish/v1/EventService/Subscriptions/")
      ...
      .methods(
        ...
        uint16_t port = 0;
        std::string path;
        if (!crow::utility::validateAndSplitUrl(destUrl, uriProto, host, port,
                                                path)) {
 ...
 boost::urls::error_code ec;
        ...
        if (msgIds) {
          ...
          for (const std::string &id : *msgIds) {
            ...
            for (const std::string &it : registryPrefix) {
              const boost::beast::span<const redfish::registries::MessageEntry>
                  registry = redfish::registries::getRegistryFromPrefix(it);

              if (std::any_of(registry.cbegin(), registry.cend(),
        ...
        if (protocol == "SNMPv2c") {
          ...
          checkSnmpTrapClient(asyncResp, host, std::to_string(port), destUrl,
                              subValue);
          return;
        }

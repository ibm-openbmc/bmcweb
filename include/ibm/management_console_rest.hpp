                           ...
                           const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    if (ec)
    {
        ...
        BMCWEB_LOG_ERROR("saveConfigFile: Failed to prepare save-area "
    ...
    for (const auto& it : iter)
    {
        if (!std::filesystem::is_directory(it, ec))
        {
            if (ec)
            {
                ...
                BMCWEB_LOG_ERROR("saveConfigFile: Failed to find save-area "
                                 "directory . ec : {}",
                                 ec.message());
                return false;
            }
    ...
    const std::vector<uint32_t>& listTransactionIds)
{
    ...
    auto statusRelease =
        std::get<crow::ibm_mc_lock::RcReleaseLock>(varReleaselock.second);

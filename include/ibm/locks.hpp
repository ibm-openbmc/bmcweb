...
#include <boost/endian/conversion.hpp> // NOLINT
...
// Rc is an abbreviation for return code.
using Rc =
    std::pair<bool, std::variant<uint32_t, std::pair<uint32_t, LockRequest>>>;
using RcReleaseLock = std::pair<bool, std::pair<uint32_t, LockRequest>>;
using RcGetLockList =
...
using RcReleaseLockApi = std::pair<bool, std::variant<bool, RcReleaseLock>>;
...
class Lock
{
  ...
  protected:
    ...
    virtual RcReleaseLock isItMyLock(const ListOfTransactionIds& refRids,
                                     const SessionFlags& ids);

    /*
     * This function validates the list of transactionID's and returns false
  ...
  public:
    ...
    /*
     * This function releases all the locks obtained by a particular
     * session.
     */
...
inline RcGetLockList Lock::getLockList(const ListOfSessionIds& listSessionId)
{
    ...
    // return the list of lock records pertaining to the given
    ...
    if (!lockTable.empty())
    {
        ...
        while (it != lockTable.end())
        {
            if (!it->second.empty())
            {
                ...
                {
                    BMCWEB_LOG_DEBUG(
                        "Remove the lock from the locktable having sessionID={}",
                        sessionId);
...
inline RcReleaseLock Lock::isItMyLock(const ListOfTransactionIds& refRids,
                                      const SessionFlags& ids)
{

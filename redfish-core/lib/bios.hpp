    ...
    const std::string& systemName)
{
    ...
    asyncResp->res.jsonValue["@Redfish.Settings"]["@odata.type"] = "#Settings.v1_3_0.Settings";
    asyncResp->res.jsonValue["@Redfish.Settings"]["SettingsObject"]["@odata.id"] = "/redfish/v1/Systems/system/Bios/Settings";
    ...
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    asyncResp->res.jsonValue["RegistryEntries"]["Attributes"] = nlohmann::json::array();
    ...
    dbus::utility::getProperty<BiosBaseTableType>(
        ...
        [asyncResp](const boost::system::error_code& ec,
            ...
            if (ec)
            {
                ...
                messages::resourceNotFound(asyncResp->res, "Registries/Bios", "Bios");
                return;
            }
            ...
            for (const BiosBaseTableItemType& item : baseBiosTable)
            {
                ...
                const std::string& itemType = std::get<biosBaseAttrType>(item.second);
                ...
                attributeItem["ReadOnly"] = std::get<biosBaseReadonlyStatus>(item.second);
                attributeItem["DisplayName"] = std::get<biosBaseDisplayName>(item.second);
                attributeItem["HelpText"] = std::get<biosBaseDescription>(item.second);

                if (!std::get<biosBaseMenuPath>(item.second).empty())
                {
                    attributeItem["MenuPath"] = std::get<biosBaseMenuPath>(item.second);
                }
    ...
    const std::string& systemName)
{
        ...
        dbus::utility::getProperty<BiosBaseTableType>(
            ...
            "/xyz/openbmc_project/bios_config/manager",
            "xyz.openbmc_project.BIOSConfig.Manager", "BaseBIOSTable",         
            [asyncResp, attrsJson, systemName](const boost::system::error_code& ec,
                        const BiosBaseTableType& baseBiosTable) {
            if (ec)
            {

...
using RedfishUriListType =
    std::vector<std::tuple<std::vector<std::string>, std::string>>;

const RedfishUriListType redfishUriList = {
    {std::vector<std::string>(processorInterfaces.begin(),
                              processorInterfaces.end()),
     std::string(processorRedfishUri)},
    {std::vector<std::string>(dimmInterfaces.begin(), dimmInterfaces.end()),
     std::string(memoryRedfishUri)},
    {std::vector<std::string>(procCoreInterfaces.begin(),
                              procCoreInterfaces.end()),
     std::string(processorCoreRedfishUri)}};
...
/**
 * @brief API used to fill the respective redfish resource "Id" in the given
 *        redfishUri by using the given dbus object path which represents the
 *        given redfishUri and assign the filled redfish uri into
 ...
 * @param[in] redfishUri - The redfish uri to fill with its "Id".
 *
 * @return The redfish response with "OriginOfCondition" property of
 *         LogEntry schema if success else nothing in redfish response.
 *
 * @note - This function will get the associated redfish uri prefix of "Id"
 *         ("<str>") and look at the predefined redfish list (which will
 *         contains the pair <DBus Interfaces (which represents redfish
 *         resource), RedfishUri> of elements) to get the dbus interfaces and
 *         that is used to get the dbus objects which will compare with the
 *         given dbus object path to fill the respective uri Id in the
 *         given redfishUri.
 *       - This function will run recursively until filling all Ids in given
 *         given redfishUri.
 */
inline void
    fillRedfishUriWithIds(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                          const size_t entryJsonIdx,
                          const sdbusplus::message::object_path& dbusObjPath,
                          std::string& redfishUri)
{

    std::string uriIdPattern{"<str>"};
    if (auto uriIdPos = redfishUri.rfind(uriIdPattern);
        uriIdPos != std::string::npos)
    {
        // Reduce by "1" from uriIdPos to get the redfish uri of uriIdPattern
        // For example,
        // "/redfish/v1/Systems/system/Processors/<str>/SubProcessors/core0"
        //                                       ^
        auto parentRedfishUri = redfishUri.substr(0, uriIdPos - 1);
        auto parentRedfishUriIt =
            std::find_if(redfishUriList.begin(), redfishUriList.end(),
                         [parentRedfishUri](const auto& ele) {
                             return parentRedfishUri == std::get<1>(ele);
                         });

        if (parentRedfishUriIt == redfishUriList.end())
        {
            BMCWEB_LOG_ERROR
                << "Failed to get parent Redfish URI DBus interface for the "
                   "identified Redfish URI: "
                << redfishUri
        ...
        crow::connections::systemBus->async_method_call(
            [asyncResp, entryJsonIdx, dbusObjPath, uriIdPos, uriIdPattern,
             parentRedfishUri,
             redfishUri](const boost::system::error_code ec,
                         const std::vector<std::string>& objects) mutable {
                if (ec || objects.empty())
                {
                    BMCWEB_LOG_ERROR << "Failed to get parent DBus path for "
                                        "identified parent Redfish URI: "
                                     << parentRedfishUri
                ...
                {
                    if (dbusObjPath.str.find(object) != std::string::npos)
                    {
                        uriIdObj = object;
                        break;
                    }
                ...
                redfishUri.replace(
                    ...
                    getIsolatedHwItemId(
                        sdbusplus::message::object_path(uriIdObj)));

                if (redfishUri.find(uriIdPattern) == std::string::npos)
                {
                    nlohmann::json& entryJson =
                        (entryJsonIdx > 0
                             ? asyncResp->res
                                   .jsonValue["Members"][entryJsonIdx - 1]
                             : asyncResp->res.jsonValue);

                    entryJson["Links"]["OriginOfCondition"] = {
                        {"@odata.id", redfishUri}};
                }
                else
                {
                    fillRedfishUriWithIds(asyncResp, entryJsonIdx, dbusObjPath,
                                          redfishUri);
                }
            },
            "xyz.openbmc_project.ObjectMapper",
            ...
            std::get<0>(*parentRedfishUriIt));
    }
}

/**
 * @brief API used to fill the respective redfish uri (which will get by using
 *        the given dbus object path) into "OriginOfCondition" property of
 *        LogEntry schema.
 *
 * @param[in] asyncResp - The redfish response to return.
 * @param[in] entryJsonIdx - The json entry index to add isolated hardware
 *                            details in the appropriate entry json object.
 * @param[in] dbusObjPath - The DBus object path which represents redfishUri.
 * @param[in] redfishUri - The iterator to fill redfish uri with its "Id".
 *
 * @return The redfish response with "OriginOfCondition" property of
 *         LogEntry schema if success else nothing in redfish response.
 *
 * @note - This function will make sure to assign the redfish uri with Ids
 *         if it contains "<str>".
 *       - This function will run recursively until finding the redfish uri
 *         from the list by using the given dbus object path.
 */
inline void
    getRedfishUriFromList(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
                          const size_t entryJsonIdx,
                          const sdbusplus::message::object_path& dbusObjPath,
                          RedfishUriListType::const_iterator redfishUriIt)
{
    ...
    {
        BMCWEB_LOG_ERROR
            << "Failed to get Redfish URI for the given DBus object path: "
            << dbusObjPath.str;
        return;
    }

    crow::connections::systemBus->async_method_call(
        [asyncResp, entryJsonIdx, dbusObjPath,
         redfishUriIt](const boost::system::error_code ec,
                       const GetObjectType& objType) mutable {
            if (ec || objType.empty())
            {
                // Look the next element from redfish uri list
                getRedfishUriFromList(asyncResp, entryJsonIdx, dbusObjPath,
                                      std::next(redfishUriIt));
            }
            else
            {
                std::string redfishUri = std::get<1>(*redfishUriIt) + "/" +
                                         getIsolatedHwItemId(dbusObjPath);
                if (redfishUri.find("<str>") == std::string::npos)
                {
                    nlohmann::json& entryJson =
                        (entryJsonIdx > 0
                             ? asyncResp->res
         ...
         entryJsonIdx](const boost::system::error_code ec,
            ...
            crow::connections::systemBus->async_method_call(
                [asyncResp, dbusObjPath, entryJsonIdx](
                    ...
                    const std::variant<std::string>& prettyName) mutable {
                    if (ec)
                    {
                        BMCWEB_LOG_ERROR << "DBUS response error ["
                                         << ec.value() << " : " << ec.message()
                                         << "] when tried to get the "
                                            "PrettyName of isolated hareware: "
                                         << dbusObjPath.str;
                        messages::internalError(asyncResp->res);
                        return;
                    }

                    const std::string* prettyNameVal =
                        std::get_if<std::string>(&(prettyName));
                    if (prettyNameVal == nullptr)
                    {
                        BMCWEB_LOG_ERROR << "Failed to get the PrettyName "
                                         << "from object: " << dbusObjPath.str;
                        messages::internalError(asyncResp->res);
                        return;
                    }
                    nlohmann::json& entryJson =
                    ...
                    entryJson["Message"] = *prettyNameVal;
                },
                objType[0].first, dbusObjPath.str,
                "org.freedesktop.DBus.Properties", "Get",

                           ...
                           const std::string& guardType)
{
    ...
    const redfish::registries::Message* msgReg =
        registries::getMessage("OpenBMC.0.6.GardRecord");
    if (msgReg == nullptr)
    {
        BMCWEB_LOG_ERROR(
            "Failed to get the GardRecord message registry to add in the condition");
        messages::internalError(asyncResp->res);
        return;
    }
    ...
    const size_t entryJsonIdx, GetManagedObjectsType::const_iterator& dbusObjIt)
{
    ...
    entryJson["MessageId"] = "OpenBMC.0.6.GardRecord";

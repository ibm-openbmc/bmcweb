        ...
        [asyncResp, fanId,
         chassisId](const boost::system::error_code ec,
            ...
            for (const auto& tempsensorpath : sensorpaths)
            {
                ...
                {
                    BMCWEB_LOG_DEBUG << "This is not a fan-related sensor  "
                                     << leaf;
                    continue;
                }
            }
  ...
  private:
               ...
               const std::vector<std::string>& params) override
    {
        ...
        auto getChassisId = [asyncResp, chassisId, fanId](
            ...
            // chassis association
            getValidfanId(asyncResp, chassisId, fanId, std::move(getFanId));
        };
        redfish::chassis_utils::getValidChassisID(asyncResp, chassisId,
                                                  std::move(getChassisId));
    }
                 ...
                 const std::vector<std::string>& params) override
    {
        if (params.size() != 2)
        {
            BMCWEB_LOG_ERROR << "Not valid params pass";
            messages::internalError(asyncResp->res);
            return;
        }

                                          ...
                                          "Entry")
                {
                        ...
                        !eventId.empty())
                    {
                        taskData->messages.emplace_back(messages::taskAborted(
                            index, message, addDataStr, eventId));
                        return;
                    }
                }
            }
        ...
        nlohmann::json& updateSvcConUpdate =
            asyncResp->res
                .jsonValue["Actions"]["Oem"]["#OemUpdateService.v1_0_0."
                                             "ConcurrentUpdate"];
        updateSvcConUpdate["target"] = "/redfish/v1/UpdateService/Actions/Oem/"
                                       "OemUpdateService.ConcurrentUpdate";
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/UpdateService/FirmwareInventory/<str>/")
        ...
        crow::connections::systemBus->async_method_call(
                     ...
                     obj : subtree)
            {
                ...
                sw_util::getSwMinimumVersion(asyncResp, obj.second[0].first);
            }
            if (!found)
            {

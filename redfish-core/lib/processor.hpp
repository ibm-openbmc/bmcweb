...
{
    asyncResp->res.jsonValue["@odata.type"] = "#Processor.v1_12_0.Processor";
    asyncResp->res.jsonValue["@odata.id"] =
        "/redfish/v1/Systems/system/Processors/" + processorId;
    asyncResp->res.jsonValue["SubProcessors"] = {
                                ...
                                const std::string& coreId)
{
    ...
    auto callback =
        [aResp, processorId, coreId](const std::string& cpuPath) {
        crow::connections::systemBus->async_method_call(
...
inline void requestRoutesProcessorCollection(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Processors/")
        ...
        crow::connections::systemBus->async_method_call(
            ...
            "xyz.openbmc_project.ObjectMapper",
            "/xyz/openbmc_project/object_mapper",
>>>>>>> 58bad7bd (redfish-core: Core: Enabled the isolation feature)
            "xyz.openbmc_project.ObjectMapper", "GetSubTreePaths",
            "/xyz/openbmc_project/inventory", 0,
            std::array<const char*, 1>{
                "xyz.openbmc_project.Inventory.Item.Cpu"});
    ...
    {
        BMCWEB_LOG_DEBUG
            << "Get available system cpu core resources by service.";
    ...
    inline void getSubProcessorData(
        const std::shared_ptr<bmcweb::AsyncResp>& aResp,
        const std::string& processorId, const std::string& coreId)
    {
        BMCWEB_LOG_DEBUG << "Get available system sub processor resources.";

        auto callback = [aResp, processorId,
        ...
        const std::string& processorId)
    {
        auto callback = [aResp, processorId](const std::string& cpuPath) {
            crow::connections::systemBus->async_method_call(
                ...
                "xyz.openbmc_project.ObjectMapper",
                "/xyz/openbmc_project/object_mapper",
                "xyz.openbmc_project.ObjectMapper", "GetSubTreePaths", cpuPath,
                0,
                std::array<const char*, 1>{
                    "xyz.openbmc_project.Inventory.Item.CpuCore"});
        };
        ...
        const sdbusplus::message_t& msg)
    {
        ...
        // to parse the error string. Some of these D-Bus -> Redfish
        // translations are a stretch, but it's good to try to communicate
                   ...
                   "xyz.openbmc_project.Common.Error.InvalidArgument") == 0)
        {
            ...
            messages::propertyValueIncorrect(
                resp->res, "AppliedOperatingConfig/@odata.id", setPropVal);
                        ...
                        "xyz.openbmc_project.Common.Error.NotAllowed") == 0)
        {
            // Service indicates we can never change the config for this
            // processor.
            messages::propertyNotWritable(resp->res, "AppliedOperatingConfig");
        }
        else if (strcmp(dbusError->name,
                        "xyz.openbmc_project.Common.Error.Unavailable") == 0)
        {
            // Service indicates the config cannot be changed right now, but
            // maybe in a different system state.
            messages::resourceInStandby(resp->res);
        }
        else
        {
    ...
    /**
     * Handle the PATCH operation of the AppliedOperatingConfig property. Do
     * basic validation of the input data, and then set the D-Bus property.
     *
     * @param[in,out]   resp            Async HTTP response.
     * @param[in]       processorId     Processor's Id.
     * @param[in]       appliedConfigUri    New property value to apply.
        ...
        const dbus::utility::MapperServiceMap& serviceMap)
    {
        ...
        if (!appliedConfigUri.starts_with(expectedPrefix) ||
            expectedPrefix.size() == appliedConfigUri.size())
        {
            messages::propertyValueIncorrect(resp->res,
        ...
        std::string configBaseName =
            appliedConfigUri.substr(expectedPrefix.size());
        sdbusplus::message::object_path configPath(cpuObjectPath);
        configPath /= configBaseName;
    ...
    /**
     * Find the D-Bus object representing the requested Processor, and call the
     * setProcessorData with the results. If matching object is not found, add
     * 404 error to response and don't call the setProcessorData.
     *
     * @param[in,out]   resp                            Async HTTP response.
     * @param[in]       processorId                     Redfish Processor Id.
     * @param[in]       locationIndicatorActive         Value of the property
     */
    inline void setProcessorObject(
        const std::shared_ptr<bmcweb::AsyncResp>& resp,
        const std::string& processorId,
        std::optional<bool> locationIndicatorActive)
    {
    ...
    {
        BMCWEB_ROUTE(
            app, "/redfish/v1/Systems/<str>/Processors/<str>/OperatingConfigs/")
            .privileges(redfish::privileges::getOperatingConfigCollection)
            .methods(boost::beast::http::verb::get)(
                [&app](const crow::Request& req,
                       const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    {
        BMCWEB_ROUTE(
            app, "/redfish/v1/Systems/system/Processors/<str>/SubProcessors")
            .privileges(redfish::privileges::getProcessorCollection)
            .methods(boost::beast::http::verb::get)(
                [](const crow::Request&,
                   const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,

...
inline void
getLicenseEntryCollection(const std::shared_ptr<bmcweb::AsyncResp> &asyncResp) {
...
inline void translateLicenseTypeDbusToRedfish(
  ...
  if (licenseType == "com.ibm.License.Entry.LicenseEntry.Type.Purchased") {
    asyncResp->res.jsonValue["LicenseType"] = "Production";
             ...
             "com.ibm.License.Entry.LicenseEntry.Type.Prototype") {
            ...
            if (interfaceMap.first == "com.ibm.License.Entry.LicenseEntry") {
              for (const auto &propertyMap : interfaceMap.second) {
                if (propertyMap.first == "Name") {
                  ...
                  deviceNumPtr = std::get_if<uint32_t>(&propertyMap.second);
                  ...
                  expirationTime = static_cast<std::time_t>(*timePtr);
                } else if (propertyMap.first == "SerialNumber") {
                  serialNumPtr = std::get_if<std::string>(&propertyMap.second);
